...
1. Client -> Server commuincation architecture 
...
3. HTTPS: does the same thing as above by doing encryption of request and response 
...
5. A port number: Port number is a logical number used to identify a specific network service/application on a device.
...
6. servelet : java class runs in servelet container 
   servelet container : geberally called as server ( which runs in JVM)
                       ex: tomcat, jetty
